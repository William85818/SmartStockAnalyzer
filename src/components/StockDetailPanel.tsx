    ...
    <motion.div key="detailPanel" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-[#111624] p-6 rounded-2xl border border-slate-800 shadow-xl">
        ...
        <div className="text-left md:text-right">
          ...
          {stock.priceUpdatedAt && <p className="text-[11px] text-slate-500 mt-1.5 font-mono">資料時間：{new Date(stock.priceUpdatedAt).toLocaleString('zh-TW', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>}
        </div>
      </header>
      ...
      <div className="bg-[#111624] rounded-2xl border border-slate-800 shadow-lg flex flex-col overflow-hidden relative w-full">
        ...
        <div className="p-6 lg:p-8">
          ...
          <p className="text-sm text-slate-500 font-mono mb-8 flex items-center gap-2">
            Generated by AlphaFlow AI • 
            <span className="text-lg font-bold text-emerald-400">{stock.actionAdvice.aiConfidence}% Confidence</span>
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111624] p-6 rounded-2xl border border-slate-800 shadow-lg relative">
          <div className="absolute top-0 right-0 p-6 opacity-10 pointer-events-none overflow-hidden rounded-2xl w-full h-full">
            <Target className="w-24 h-24 absolute -top-4 -right-4" />
          </div>
          <h3 className="text-lg font-bold text-white mb-5 flex items-center gap-2 relative z-10 py-1 leading-relaxed">
